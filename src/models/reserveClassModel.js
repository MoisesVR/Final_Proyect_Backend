...
const getReserveClassId = async (id) => {
    try {
        const query = {
            text: "SELECT class_reserve.id, class_reserve.date, class_reserve.hour, class.name, class_reserve.id FROM class_reserve INNER JOIN class ON class.id = class_reserve.id_class WHERE id_user = $1",
            values: [id],
        }
